...
$("#CmbSucursalDestino").change(function () {

    //    var agenciaLogueo = jQuery.trim($('#lblAgencia1').text())

    //if (agenciaLogueo == '00001') {
    var of = jQuery.trim($('#CmbOficinaOrigen').val());
    if (of != "") {
        ValidarSucursalTieneOficina();
    }
    //}
});
...
$("#CmbOficinaOrigen").change(function () {


    var cadenaCOdigo = jQuery.trim($('#CmbOficinaOrigen').val());
    var icadena = cadenaCOdigo.length;
    var tempCodigo = "";
    var codigoSubdeposito = "";
    var codigoOficina = "";
    var EsOF = "0";

    for (var i = 0; i < icadena; i++) {

        tempCodigo = cadenaCOdigo[i];
        if (tempCodigo != "_") {

            if (EsOF == "0") {
                codigoSubdeposito = codigoSubdeposito + tempCodigo;
            } else {
                codigoOficina = codigoOficina + tempCodigo;
            }
        } else {
            EsOF = "1";
        }
    ...
    $("#hdSubdepositoSucursal").val(codigoSubdeposito);
    jQuery.trim($('#lblSubdeposito').text(codigoSubdeposito));

    CargarOficinasDestino($("#lblIdEmpresa").text(), codigoSubdeposito);
});
...
$('#txtVolumen').focusout(function () {
    ...
    if (VolumenActual != "") {
        ...
        //Solo si es cuentacorriente se valida cantidades en rampa(lastro)
        if (esCtacorriente == 'S') {
...
//Función de Cargar Oficinas segun la sucursal
function CargarOficinas(idEmpresa) {

    var datos = JSON.stringify({ IdEmpresa: $("#lblIdEmpresa").text(), agencia: jQuery.trim($('#CmbSucursalDestino').val()), Subdeposito:"0", ConectarA: jQuery.trim($('#lblBd').text()) });
    //var datos = JSON.stringify({ IdEmpresa: idEmpresa, ConectarA: jQuery.trim($('#lblBd').text()) });
    $.ajax({
        url: 'RemitoAgenciaWeb.aspx/OficinasPorSucursal',
        data: datos,
        error: function (objeto, quepaso, otroobj) {
            mostrarError(objeto, quepaso, otroobj);
        },
        success: function (response) {
            var Tdato = (typeof response.d) == 'string' ? eval('(' + response.d + ')') : response.d;
            if (Tdato != null) {
                $('#CmbOficinaOrigen').empty();
                $('#CmbOficinaOrigen').append('<option value="-1" selected="selected">Seleccione...</option>');
                $.each(Tdato, function (i, v) {
                    $('#CmbOficinaOrigen').append('<option value="' + v.CodigoOfina + '">' + v.Descripcion + '</option>');
                });
            }
        }
    });
...
//Función de Cargar Oficinas segun la sucursal
function CargarOficinasDestino(idEmpresa,codigoSubdeposito) {


    var datos = JSON.stringify({ IdEmpresa: $("#lblIdEmpresa").text(), agencia: jQuery.trim($('#CmbSucursalDestino').val()), Subdeposito: codigoSubdeposito, ConectarA: jQuery.trim($('#lblBd').text()) });
    //var datos = JSON.stringify({ IdEmpresa: idEmpresa, ConectarA: jQuery.trim($('#lblBd').text()) });
    $.ajax({
        url: 'RemitoAgenciaWeb.aspx/OficinasPorSucursal',
        data: datos,
        ...
        success: function (response) {
            var Tdato = (typeof response.d) == 'string' ? eval('(' + response.d + ')') : response.d;
            if (Tdato != null) {
                $('#CmbOficinaDestino').empty();
                $('#CmbOficinaDestino').append('<option value="-1" selected="selected">Seleccione...</option>');
                $.each(Tdato, function (i, v) {
                    $('#CmbOficinaDestino').append('<option value="' + v.CodigoOfina + '">' + v.Descripcion + '</option>');
                });
            }
        }
    });
...
//Función que determina si una Ruta es Rampa
function ValidarSucursalTieneOficina() {

    var data = JSON.stringify({ IdEmpresa: $("#lblIdEmpresa").text(), Agencia: jQuery.trim($('#CmbSucursalDestino').val()), ConectarA: jQuery.trim($('#lblBd').text()) });
    $.ajax({
        url: 'RemitoAgenciaWeb.aspx/AgenciaTieneOficinas',
        data: data,
        success: function (response) {
            var Tdato = (typeof response.d) == 'string' ? eval('(' + response.d + ')') : response.d;
            if (Tdato == true) {
                //aqui cargar combos de oficinas
                //  $("#of_origen").show("slow");
                $("#of_destino").show("slow");

                // CargarOficinas($("#lblIdEmpresa").text());
                CargarOficinasDestino($("#lblIdEmpresa").text(), jQuery.trim($('#lblSubdeposito').text()));

            } else {

                //$("#of_origen").hide("slow");

                $("#of_destino").hide("slow");
                jQuery.trim($('#CmbOficinaDestino').val(''));
                //                var subdepositoLogueo = $("#hdSubdepositoLogueo").val();

                //                $("#hdSubdepositoSucursal").val(subdepositoLogueo);
                //                jQuery.trim($('#lblSubdeposito').text(subdepositoLogueo));
...
//    }
    ...
    var cadenaCOdigoD = jQuery.trim($('#CmbOficinaDestino').val());
    var icadenaD = cadenaCOdigoD.length;
    var tempCodigoD = "";
    var codigoSubdepositoD = "";
    var codigoOficinaD = "";
    var EsOFD = "0";

    for (var i = 0; i < icadenaD; i++) {

        tempCodigoD = cadenaCOdigoD[i];
        if (tempCodigoD != "_") {

            if (EsOFD == "0") {
                codigoSubdepositoD = codigoSubdepositoD + tempCodigoD;
            } else {
                codigoOficinaD = codigoOficinaD + tempCodigoD;
            }
        } else {
            EsOFD = "1";
        }
    ...
    registro.OficinaDestino = codigoOficinaD;// jQuery.trim($('#CmbOficinaDestino').val());
    registro.OficinaOrigen = codigoOficina;// jQuery.trim($('#CmbOficinaOrigen').val());
...
//    }
  //  jAlert('Ok');
   // var ofiL = codigoSubdeposito;
    var subd = $("#hdSubdepositoSucursal").val();
...
function ConsultarCilindrosPatrimonio() {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "RemitoAgenciaWeb.aspx/Cilindros_a_transferir",
        data: JSON.stringify(params),
        dataType: "json",
        async: false,
        success: function (response, textStatus) {
...
$('#btnGuardarAsig').click(function () {
    ...
    $.ajax({
        url: 'RemitoAgenciaWeb.aspx/GrabarAsignacion',
        data: data,
        success: function (response) {
