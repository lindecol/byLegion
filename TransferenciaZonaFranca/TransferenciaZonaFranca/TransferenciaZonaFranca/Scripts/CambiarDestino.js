...
$('#txtRemito').focusout(function () {
    ...
    if ($('#txtRemito').val() != "") {
        ...
        ConsultarRemitoInter($('#txtRemito').val());


        var idemp = jQuery.trim($('#lblEmpresa').text());
...
function ConsultarRemitoInter(remito) {

    var strCodigoProducto = "";


    var datos = JSON.stringify({ IdEmpresa: jQuery.trim($('#lblEmpresa_').text()), Comprobante: remito, Sucursal: jQuery.trim($('#lblSucursal').text()) });
    $.ajax({
        url: 'CambiarDestino.aspx/ConsultarDetallesRemito',
        data: datos,
        success: function (response) {
            var Tdato = (typeof response.d) == 'string' ? eval('(' + response.d + ')') : response.d;
            if (Tdato.length != 0) {
                ...
                $('#lbbFecha').text(Tdato[0].FechaComprobante);
                $('#lblDestino').text(Tdato[0].AgenciaDestino);
                if (Tdato[0].Entregado != "1") {
                    $('#lblEntregado').text("NO");
                } else {
                    $('#lblEntregado').text("SI");
                }

            } else {
                jAlert('El Remito Digitado no existe o no corresponde con esta sucursal', 'Validacion');
...
$('#btnModificar').click(function () {
    ...
    var FueEntregado = jQuery.trim($('#lblEntregado').text());

    if (FueEntregado != '') {

        if (FueEntregado != 'SI') {

            if ($('#CmbSucursalDestino').val() != "-1") {
                var data = JSON.stringify({ Comprobante: $('#txtRemito').val(), Fecha: jQuery.trim($('#lbbFecha').text()), Agencia: jQuery.trim($('#CmbSucursalDestino').val()), ConectarA: jQuery.trim($('#lblBd').text()) });
                $.ajax({
                    url: 'CambiarDestino.aspx/ModificarDestino',
                    data: data,
                    success: function (response) {
                        var Tdato = (typeof response.d) == 'string' ? eval('(' + response.d + ')') : response.d;
                        if (Tdato.IdError == "01") {
                            jAlert('Remito modificado satisfactoriamente', 'Fin Proceso');

                        } else {
                            jAlert('Error modificado Remito:' + Tdato.MensajeError, 'Fin Proceso');
                        }
                    }

                })
            } else {
                jAlert('Seleccione Sucursal de destino', 'Validacion');
            }


        } else {
            jAlert('Remito Ingresado ya fue recepcionado, no se puede cambiar el destino', 'Validacion');
        }

    } else {

        jAlert('Ingrese número de remito valido', 'Validacion');
    }
