...
{
    public partial class RemitoAgenciaWeb : System.Web.UI.Page
    {
        ...
        [WebMethod]
        public static bool AgenciaTieneOficinas(int IdEmpresa, string Agencia, string ConectarA)
        {
            return (new Documento()).AgenciaTieneOficinas(IdEmpresa, Agencia, ConectarA);
        }
        ...
        public static void BorrarRegistroLoteSerie(string IdSerial, string Usuario, string ConectarA)
        {
            ...
            (new Producto()).BorrarRegistroLoteSerie(IdSerial, Usuario, ConectarA, "A");
        }
        ...
        public static float CantidadProductosEnRampa(int IdEmpresa, string Codigo, string EsPropio, string EsLleno, string Capacidad, string Agencia, string ConectarA, int SubDep)
        {
            ...
            float CantidaRampa = producto.CantidadProductosEnRampa(SubDep, Agencia, Agencia, RemitoAgenciaWeb.iGrupoId, Codigo, IdEmpresa, EsPropio, EsLleno, ConectarA);
            return CantidaRampa;
        }
        ...
        public static List<Producto> Cilindros_a_transferir(string Usuario, string ConectarA)
        {
            ...
            return (
                from o in (new Producto()).Cilindros_a_transferir(Usuario, "A", ConectarA)
                select new Producto()
                {
                    Codigo = o.Codigo,
        ...
        public static List<Producto> ConsultarCargueAutomaticoZF(int IdEmpresa, string Ruta, string Usuario, string Agencia, string ConectarA)
        {
            ...
            return Itemscargue.ConsultarCargueAutomaticoZF(IdEmpresa, Usuario.Trim(), Agencia, Ruta, ConectarA, "A");
        }
        ...
        public static List<Producto> ConsultarCargueAutomaticoZFSerialLote(string Usuario, string ConectarA)
        {
            List<Producto> lista = new List<Producto>();
            return (
                from o in (new Producto()).ConsultarCargueAutomaticoZFSerialLote(Usuario, "A", ConectarA)
                select new Producto()
                {
        ...
        public static List<Producto> ConsultarSerialLote(string Usuario, string ConectarA)
        {
            List<Producto> lista = new List<Producto>();
            return (
                from o in (new Producto()).ConsultarSerialLote(Usuario, "A", ConectarA)
                select new Producto()
                {
                    SerialPrax = o.SerialPrax,
                    Codigo = o.Codigo,
        ...
        {
            (new Talonario()).EliminarDatosTemporalesIniciales(TempIdEmpresa, Usuario.Trim(), ConectarA, "A");
        }
        ...
        {
            Ruta ruta = new Ruta();
            return ruta.EsRutaRampa(IdEmpresa, Agencia, CodigoRuta, RemitoAgenciaWeb.iGrupoId, ConectarA);
        }
        ...
        {
            return (new Producto()).GasStockeable(IdEmpresa, CodigoProducto, RemitoAgenciaWeb.iGrupoId, ConectarA);
        }
        ...
        public static void GrabarAsignacion(string producto, decimal capacidad, string Usuario, string ConectarA, int Asign, string Observacion)
        {
            ...
            grabarLote.GrabarAsignacion(producto, capacidad, Usuario, ConectarA, "A", Asign, Observacion);
        }
        ...
        {
            (new Mensaje()).IdError = "01";
            (new Producto()).GrabarSerialLoteTemp(IdSerial, sLote, Usuario, ConectarA, "A", producto);
        }

        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        [WebMethod]
        public static List<IEntity> OficinasPorSucursal(int IdEmpresa, string agencia, string Subdeposito, string ConectarA)
        {
            List<IEntity> lista = new List<IEntity>();
            if (IdEmpresa == 1)
            {
            ...
            Oficina oficina = new Oficina()
            {
                ...
                Empresa = IdEmpresa,
                NombreBD = ConectarA,
                Subdeposito = Subdeposito
            };
            return oficina.Consultar();
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!base.IsPostBack)
            {
                ...
                if (RecargarPagina == string.Empty)
                {
                    ...
                    RemitoAgenciaWeb.iGrupoId = 1;
                    RemitoAgenciaWeb.iDocumentoID = 21;
                    ...
                    if (empresa.ConsultarMultipleSesion(this.lblIdEmpresa.Text.ToString(), this.lblNombreUsuario.Text.Trim(), Conectar_a, "A"))
                    {
                    ...
                    this.cmbEmpresaDestino.Value = empresa.ConsultarEmpresaPorId(this.lblIdEmpresa.Text.ToString(), Conectar_a);
                    Documento Doc = new Documento();
                    bool SucursalTieneOF = Doc.AgenciaTieneOficinas(int.Parse(this.lblIdEmpresa.Text.ToString()), this.lblAgencia1.Text.Trim(), Conectar_a);
                    ...
                    this.hdSubdepositoLogueo.Value = iSubdepositoId.ToString();
                    Oficina ofi = new Oficina();
                    if (!SucursalTieneOF)
                    {
                        this.CmbOficinaOrigen.Visible = false;
                    }
                    else
                    {
                        string CodigoOF = string.Empty;
                        this.CmbOficinaOrigen.Visible = true;
                        ofi.Empresa = int.Parse(this.lblIdEmpresa.Text.ToString());
                        ofi.Agencia = this.lblAgencia1.Text.Trim();
                        ofi.Subdeposito = "0";
                        ofi.NombreBD = Conectar_a;
                        this.CmbOficinaOrigen.LLenarComboLista(ofi.Consultar(), "Descripcion", "CodigoOfina");
                        for (int i = 0; i < this.CmbOficinaOrigen.Items.Count; i++)
                        {
                            if (this.CmbOficinaOrigen.Items[i].Value.Contains(this.lblSubdeposito.Text.Trim()))
                            {
                                CodigoOF = this.CmbOficinaOrigen.Items[i].Value;
                            }
                        }
                        this.CmbOficinaOrigen.SelectedValue = CodigoOF;
                    }
                    Talonario clsTalonario = new Talonario()
                    {
                        GrupoId = RemitoAgenciaWeb.iGrupoId,
                        EmpresaId = Convert.ToInt32(this.lblIdEmpresa.Text.ToString()),
                        Sucursal = strAgencia,
                        DocumentoId = RemitoAgenciaWeb.iDocumentoID,
                        TipoRetorno = 2,
                        Prefijo = strAgencia,
                    ...
                    clsTalonario.EliminarDatosTemporales(Convert.ToInt32(this.lblIdEmpresa.Text.ToString()), this.lblNombreUsuario.Text.Trim(), Conectar_a, "A");
        ...
        public static Mensaje ProcesarTransaferencia(Documento registro, string Usuario, int IdEmpresa, string Agencia, string ConectarA)
        {
            ...
            {
                MsjProceso = registro.InsertarRemitoAgenciaWeb(registro, Usuario.Trim(), ConectarA, strNumeroDocumento);
            }
        ...
        public static List<Ruta> RutaTieneAlmacen(int IdEmpresa, string CodigoRuta, string Agencia, string ConectarA)
        {
            List<Ruta> rutas = new List<Ruta>();
            Ruta ruta = new Ruta();
            return ruta.RutaTieneAlmacen(IdEmpresa, Agencia, CodigoRuta, RemitoAgenciaWeb.iGrupoId, ConectarA);
        }
        ...
        public static double ValidarCantidadStock(int IdEmpresa, int Subdeposito, string Producto, string Usuario, string ConectarA)
        {
            ...
            double CantidadStock = producto.ValidarCantidadStock(IdEmpresa, Subdeposito, Producto, Usuario.Trim(), ConectarA, "A");
