...
{
    public partial class DocumentoTransferencia : System.Web.UI.Page
    {
        ...
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        [WebMethod]
        public static void BorrarRegistroSQRTemp(string Producto, double Capacidad, string Usuario, string ConectarA)
        {
            (new Mensaje()).IdError = "01";
            (new TransferenciasZF.Data.Producto()).BorrarRegistroSQRTemp(Producto, Capacidad, Usuario, ConectarA);
        }
        ...
        public static float CantidadProductosEnRampa(int IdEmpresa, string Codigo, string EsPropio, string EsLleno, string Capacidad, string Agencia, string ConectarA, int SubDep)
        {
            ...
            float CantidaRampa = producto.CantidadProductosEnRampa(SubDep, Agencia, Agencia, DocumentoTransferencia.iGrupoId, Codigo, IdEmpresa, EsPropio, EsLleno, ConectarA);
            return CantidaRampa;
        }
        ...
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        [WebMethod]
        public static List<Producto> ConsultarSQR(string Usuario, string ConectarA)
        {
            List<Producto> lista = new List<Producto>();
            return (
                from o in (new Producto()).ConsultarSQR(Usuario, "I", ConectarA)
                select new Producto()
                {
                    Codigo = o.Codigo,
                    Capacidad = o.Capacidad,
        ...
        {
            Ruta ruta = new Ruta();
            return ruta.EsRutaRampa(IdEmpresa, Agencia, CodigoRuta, DocumentoTransferencia.iGrupoId, ConectarA);
        }
        ...
        {
            return (new Producto()).GasStockeable(IdEmpresa, CodigoProducto, DocumentoTransferencia.iGrupoId, ConectarA);
        }
        ...
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        [WebMethod]
        public static void GrabarSQR(string Producto, double Capacidad, string SQR, string Usuario, string ConectarA)
        {
            (new Mensaje()).IdError = "01";
            (new TransferenciasZF.Data.Producto()).GrabarSQR(Producto, Capacidad, SQR, Usuario, ConectarA);
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!base.IsPostBack)
            {
                ...
                if (RecargarPagina == string.Empty)
                {
                    ...
                    DocumentoTransferencia.iGrupoId = 1;
                    DocumentoTransferencia.iDocumentoID = 21;
                    ...
                    {
                        GrupoId = DocumentoTransferencia.iGrupoId,
                        EmpresaId = Convert.ToInt32(this.lblIdEmpresa.Text.ToString()),
                        Sucursal = strAgencia,
                        DocumentoId = DocumentoTransferencia.iDocumentoID,
                        TipoRetorno = 2,
                        Prefijo = strAgencia,
        ...
        public static List<Ruta> RutaTieneAlmacen(int IdEmpresa, string CodigoRuta, string Agencia, string ConectarA)
        {
            List<Ruta> rutas = new List<Ruta>();
            Ruta ruta = new Ruta();
            return ruta.RutaTieneAlmacen(IdEmpresa, Agencia, CodigoRuta, DocumentoTransferencia.iGrupoId, ConectarA);
        }
        ...
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        [WebMethod]
        public static bool ValidarSalidaCilindrosLLenos(int SubdePosito)
        {
            bool validar;
            string sTipo = string.Empty;
            validar = ((new Empresa()).ValidarSalidaCilindrosLLenos(SubdePosito) != "V" ? false : true);
            return validar;
        }

        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        [WebMethod]
        public static string ValidarsiEsPPoSL(string Codigo, string Conectar_a)
        {
            return (new ProductoDao()).ValidarsiEsPPoSL(Codigo, Conectar_a);
        }
        ...
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        [WebMethod]
        public static int VerificarExiteSQR(int IdEmpresa, int SQR, string ConectarA)
        {
            return (new Producto()).VerificarExiteSQR(IdEmpresa, SQR, ConectarA);
        }
    }
}