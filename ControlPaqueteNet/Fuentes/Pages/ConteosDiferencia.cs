...
{
	/// 	<summary>
	/// Clase ConteosDiferencia.
	/// </summary>
	/// 	<remarks>
	/// Clase generada automáticamente.
	/// </remarks>
	public class ConteosDiferencia : System.Web.UI.Page
	{
		...
		protected virtual GridView grDiferencia
		{
			...
			set
			{
				...
				{
					ConteosDiferencia conteosDiferencium = this;
					this._grDiferencia.SelectedIndexChanged -= new EventHandler(conteosDiferencium.grDiferencia_SelectedIndexChanged);
				}
				this._grDiferencia = value;
				if (this._grDiferencia != null)
				{
					ConteosDiferencia conteosDiferencium1 = this;
					this._grDiferencia.SelectedIndexChanged += new EventHandler(conteosDiferencium1.grDiferencia_SelectedIndexChanged);
				}
			}
		}
		...
		[DebuggerNonUserCode]
		static ConteosDiferencia()
		{
			ConteosDiferencia.__ENCList = new ArrayList();
		}

		public ConteosDiferencia()
		{
			ConteosDiferencia conteosDiferencium = this;
			base.Load += new EventHandler(conteosDiferencium.Page_Load);
			ArrayList _ENCList = ConteosDiferencia.__ENCList;
			Monitor.Enter(_ENCList);
			try
			{
				ConteosDiferencia.__ENCList.Add(new WeakReference(this));
			}
			finally
			{
		...
		public void CrearGrilla()
		{
			...
			try
			{
				DataSet CrearProgramacion = this.wsData.CrearProgramacion(Conversions.ToString(this.Session["Conexion"]), "B", Conversions.ToInteger(this.Session["Grupo"]), Conversions.ToInteger(this.Session["Empresa"]), Conversions.ToInteger(this.Session["Usuario"]), Conversions.ToString(this.Session["Agencia"]));
				if (CrearProgramacion.Tables[0].Rows.Count > 0)
				{
					this.grDiferencia.DataSource = CrearProgramacion.Tables[0];
