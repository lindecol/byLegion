...
{
	/// 	<summary>
	/// Clase ReporteCierreLastro.
	/// </summary>
	/// 	<remarks>
	/// Clase generada automáticamente.
	/// </remarks>
	public class ReporteCierreLastro : System.Web.UI.Page
	{
		...
		protected virtual ImageButton btnScript
		{
			...
			set
			{
				...
				{
					ReporteCierreLastro reporteCierreLastro = this;
					this._btnScript.Click -= new ImageClickEventHandler(reporteCierreLastro.btnScript_Click);
				}
				this._btnScript = value;
				if (this._btnScript != null)
				{
					ReporteCierreLastro reporteCierreLastro1 = this;
					this._btnScript.Click += new ImageClickEventHandler(reporteCierreLastro1.btnScript_Click);
				}
			}
		}
		...
		protected virtual DropDownList cmbTipoProgramacion
		{
			...
			set
			{
				...
				{
					ReporteCierreLastro reporteCierreLastro = this;
					this._cmbTipoProgramacion.SelectedIndexChanged -= new EventHandler(reporteCierreLastro.cmbTipoProgramacion_SelectedIndexChanged);
				}
				this._cmbTipoProgramacion = value;
				if (this._cmbTipoProgramacion != null)
				{
					ReporteCierreLastro reporteCierreLastro1 = this;
					this._cmbTipoProgramacion.SelectedIndexChanged += new EventHandler(reporteCierreLastro1.cmbTipoProgramacion_SelectedIndexChanged);
				}
			}
		}
		...
		[DebuggerNonUserCode]
		static ReporteCierreLastro()
		{
			ReporteCierreLastro.__ENCList = new ArrayList();
		}

		public ReporteCierreLastro()
		{
			ReporteCierreLastro reporteCierreLastro = this;
			base.Load += new EventHandler(reporteCierreLastro.Page_Load);
			ArrayList _ENCList = ReporteCierreLastro.__ENCList;
			Monitor.Enter(_ENCList);
			try
			{
				ReporteCierreLastro.__ENCList.Add(new WeakReference(this));
			}
			finally
			{
		...
		protected void btnScript_Click(object sender, ImageClickEventArgs e)
		{
			...
			try
			{
				...
				if (!(this.cmbTipoProgramacion.SelectedIndex == 0 & this.cmbConteo.SelectedIndex == 0 & Microsoft.VisualBasic.CompilerServices.Operators.CompareString(this.txtFechaInicio.Text, "", false) == 0 & Microsoft.VisualBasic.CompilerServices.Operators.CompareString(this.txtFechaFin.Text, "", false) == 0))
				{
					...
					dsReporte = this.oData.Consulta_tabla_estadisticas(this.txtFechaInicio.Text, this.txtFechaFin.Text, Conversions.ToString(this.Session["Conexion"]), "pck_ocpp_reportes.pr_lista_cierre_lastro", this.cmbTipoProgramacion.SelectedValue, Conteo, Conversions.ToString(this.Session["Agencia"]), this.cmbTipoCierre.SelectedValue);
